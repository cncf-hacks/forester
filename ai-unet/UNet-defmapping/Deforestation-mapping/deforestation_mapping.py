...
def download_file(url, retries=5):
    ...
    # print(f"Keycloak token: {keycloak_token}")
    # print(f"Downloading {url}")

    header = {'Authorization': f'Bearer {keycloak_token}'}
    ...
    for attempt in range(retries):
        try:
            with session.get(url, stream=True, allow_redirects=False, timeout=(20, 180)) as response:
    ...
    #     except requests.exceptions.RequestException as e:
    #         print(f"Error during download: {e}")
    #         break  # For non-retryable errors
    # return None
...
def download_images(save_imgs, save_rgb, save_tiles, unet_weights, unet_clouds,
    ...
    for i in range(len(df)):
        ...
        # url = f"https://download.dataspace.copernicus.eu/odata/v1/Products({pr})/$value"
        # print("Url to download: ", url)
        # print("Authorization: Bearer ", keycloak_token)
        # return
        # with session.get(url, stream=True, allow_redirects=False) as response:
        # response = download_file(url)
        # if response is None:
        #     print(f"Failed to download {prName}")
        #     continue
        ...
        if os.path.isfile(path_zip_name):                  
            # extract files 
            print("Extracting files from zipfile: ", path_zip_name)
            try:
                zip_ref = zipfile.ZipFile(path_zip_name, 'r')
            except:
                print("Error extracting zipfile", path_zip_name)
                continue
            ...
            path_to_folder = save_imgs + '/' + SAFE_file_name + '/GRANULE/'
...
def download_images_old(save_imgs, save_rgb, save_tiles, unet_weights, unet_clouds,
                    class_path, class_clouds, poly_path, percentiles_forest, 
                    percentiles_clouds, boundsdata): 
...
def application(save_tiles, unet_weights, class_path, percentiles, clouds):
    ...
    results = model.predict(next(testGene),steps=steps_application, verbose=1)
